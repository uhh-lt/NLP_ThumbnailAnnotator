  ...
  <div class="clearfix mb-0">
    <div class="row">
      ...
      <div class="col-2">
        <h6>
          <b-btn
            v-b-toggle="details_collapse_id"
            class="badge badge-secondary text-monospace pointer"
          >
            Details
          </b-btn>
        </h6>
      ...
      <div class="col-2">
        <h6>
          <b-btn
            v-b-toggle="context_collapse_id"
            class="badge badge-secondary text-monospace pointer"
          >
            UniDeps
          </b-btn>
    ...
    <b-collapse
      :id="context_collapse_id"
      class="text-left text-warning"
    >
      <hr>
      <ul class="list-unstyled">
        <li v-for="c in this.captionTokenInstance.udContext">
          <code class="text-warning">{{ c.type }}({{ c.governor }},{{ c.dependent }})</code>
        </li>
      </ul>
    </b-collapse>

    <b-collapse
      :id="sense_collapse_id"
      class="text-left text-warning"
    >
      <hr>
      <ul class="list-unstyled">
        <li
          v-for="(sense, n) in this.captionTokenInstance.wordNetSenses"
          class="mb-1"
        >
          <code
            :id="sense_collapse_id + n"
            class="text-warning sense"
          >{{ sense }}</code>
    ...
    <div
      v-if="predictionReady && predictedThumbnailUrl !== null"
      class="row"
    >
      <div class="col-4 text-center sense">
        <img
          :src="predictedThumbnailUrl"
          class="img-thumbnail"
          :alt="predictedThumbnailUrl"
        >
      </div>

      <div class="col-8 text-left">
        <div
          v-for="(val, cat, i) in predictedCategories"
          :id="cat + '_popover_target'"
        >
          <div
            v-if="i === 0"
            class="badge-group w-100 mb-2"
          >
            <span
              v-if="i === 0"
              class="badge badge-success text-left w-50"
            >
              {{ cat }}
            </span>
    ...
    predict (captionTokenId) {
      ...
      setTimeout(() => {
        axios.get(this.$hostname + '/predict?captionTokenId=' + captionTokenId + '&accessKey=' + this.request_access_key).then(response => {
          ...
          const predictedThumbnailUrl = Object.keys(response.data)[0]
          const categoryPrediction = Object.values(response.data)[0]
